...
ice_ASRPublish = False
for p in icePaths:
	print 'Trying', p, 'to load ASRPublish.ice'
	if os.path.isfile(p+'/ASRPublish.ice'):
		print 'Using', p, 'to load ASRPublish.ice'
		preStr = "-I/opt/robocomp/interfaces/ -I"+ROBOCOMP+"/interfaces/ " + additionalPathStr + " --all "+p+'/'
		wholeStr = preStr+"ASRPublish.ice"
		Ice.loadSlice(wholeStr)
		ice_ASRPublish = True
		break
if not ice_ASRPublish:
	print 'Couldn\'t load ASRPublish'
	sys.exit(-1)
